...
compiler to an LLVM IR-targeted compiler. This choice was not only efficient
from a maintenance standpoint, but it is also a useful
...
variable, in that they can have any name conforming to the naming
rules, and they are unlimited in number. LLVM does, however, requires
...
LLVM IR you can not ``fall through'' from one block to the next
without using an explicit terminator instruction specifying a jump to
the next block's label. This strictly-defined block-based organization
...
labels at the start of each block and explicit branching (or return
statements) at the end of each block.
...
optimizations in the future, especially if the SSA conversion pass is
...
In general, LLVM feels like a well though out Intermediate Language
