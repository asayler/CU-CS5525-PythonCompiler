...
The LLVM instruciton selection requires several more features to be
...
learn the nuances of the LLVM Intermediate Representation that we will
have to address in our compiler, we began converting out test programs
to LLVM IR by hand and compiling them. There are several new issues
that we must address in our LLVM instruciton selection.
...
keeping track of type much more straightforward, as the type checking
and bit shifting are accomplished by the C helper files.
...
.ll file that we output which contains the declarations for out
...
constant \cite{llvm-discussions}. The options avaiable are to perform
constant propogation, or to declare each constant as an internal
constant, like so:
\begin{verbatim}
@V1 = constant i32 5
\end{verbatim}
...
find-replace. We will consider this option as an optimization.
...
\cite{clang.llvm.org}. We then link the bitcode to the bitcode
generated from the .ll output from our compiler. This gives us a
bitcode file that we are capable of running in the LLVM interpreter or
being fully compiled to an executable.  The process for compiling is
show in Listing \ref{lst:compiling.sh}.
